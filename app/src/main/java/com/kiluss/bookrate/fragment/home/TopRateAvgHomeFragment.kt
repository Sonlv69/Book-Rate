...
import com.kiluss.bookrate.adapter.BookPreviewTopRateAvgAdapter
...
class TopRateAvgHomeFragment : Fragment(), BookPreviewAdapterInterface {
    private lateinit var layoutManager: GridLayoutManager
    private var bookLists: ArrayList<TopRateAvg> = arrayListOf()
    private var _binding: FragmentMostRecentHomeBinding? = null
    private val binding get() = _binding!!
    private lateinit var bookAdapter: BookPreviewTopRateAvgAdapter
    ...
    override fun onCreateView(
        ...
        bookAdapter = BookPreviewTopRateAvgAdapter(arrayListOf(), requireContext(), this)
        loadData()
        return binding.root
    }

    private fun initView() {
        bookAdapter.setData(bookLists)
        setRVLayoutManager()
    }
    ...
    private fun loadData() {
        getLoginResponse(requireContext()).token?.let {
            RetrofitClient.getInstance(requireContext()).getClientAuthorized(it)
                .create(BookService::class.java).getStatisticalRate()
                .enqueue(object : Callback<ArrayList<TopRateAvg>?> {
                    override fun onResponse(
                        call: Call<ArrayList<TopRateAvg>?>,
                        response: Response<ArrayList<TopRateAvg>?>
                    ) {
                        _binding?.let {
                            when {
                                response.code() == 404 -> {
                    ...
                    override fun onFailure(call: Call<ArrayList<TopRateAvg>?>, t: Throwable) {
                        _binding?.let {
